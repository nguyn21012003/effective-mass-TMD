from copy import copy, deepcopy
from os import defpath
from time import time

import matplotlib.pyplot as plt
import munkres
import numpy as np
import pandas as pd
from numpy._core.fromnumeric import shape
from pandas.core.construction import range_to_ndarray
from tqdm import tqdm


def track_eigenvalues(D, V):
    # n la tong so ma tran co cung size p x p
    # size cua D la nxp
    # D la ma tran tri rieng da cheo hoa cua n Hamiltonian co size la p x p = 3q x 3q
    # V la ma tran ham rieng da cheo hoa cua n Hamiltonian co size la p x p = 3q x 3q
    Dshape = np.shape(D)
    n = Dshape[0]  # Number of Hamiltonian
    p = Dshape[-1]  # Number of Values/Vectors
    # the initial eigenvalues/vectors in nominal order
    Dseq = np.zeros([n, p])  # De luu ma tran tri rieng moi theo thu tu n voi size la p
    Vseq = np.zeros((n, p, p), dtype=complex)  # De luu ma tran vector rieng moi theo thu tu n voi size la pxp

    for i in tqdm(range(0, n), desc="sort by amplitude of real(D) "):
        tags = np.argsort(D[i])
        Dseq[i] = D[i][tags]
        Vseq[i] = V[i][:, tags]
    # now, treat each eigenproblem in sequence (after the first one.)
    m = munkres.Munkres()
    for i in tqdm(range(1, n), desc="sort using munkres"):
        # compute distance between systems
        D1 = Dseq[i - 1]
        D2 = Dseq[i]
        V1 = Vseq[i - 1]
        V2 = Vseq[i]
        dist = 1 - np.abs(V1.T @ V2)
        distance = dist * (np.sqrt(distancematrix(D1, D2) ** 2))

        # Is there a best permutation? use munkres.
        reorder = m.compute(distance)
        reorder = [coord[1] for coord in reorder]

        Dseq[i] = Dseq[i, reorder]
        Vseq[i] = Vseq[i][:, reorder]

        # also ensure the signs of each eigenvector pair
        # were consistent if possible
        S = np.squeeze(np.real(np.sum(Vseq[i - 1] * Vseq[i], 0))) < 0

        Vseq[i] = Vseq[i] * (-S.astype(int) * 2 - 1)

    return Dseq


# def track_eigenvalues(EigenArrInit, Barr):
#
#     keys = []
#     for key in EigenArrInit:
#         keys.append(key)
#
#     for i in keys[6:12]:
#         plt.plot(Barr, EigenArrInit[i], label=f"{i}")
#     plt.legend()
#     plt.show()
#     return None


def distancematrix(vec1, vec2):
    """simple interpoint distance matrix"""
    return np.abs(vec1[:, np.newaxis] - vec2[np.newaxis, :]).T


def main():
    NNdir = "./Sun-08-24/NN/"
    NNdirVectors = "./Mon-08-25/NN/"

    dataframeValues = pd.read_csv(NNdir + "3band_Lambda2q_dataHofstadterButterfly_q_797_MoS2_GGA_G.dat")

    dataframeVectors = pd.read_csv(NNdirVectors + "3band_VectorLambda2q_dataHofstadterButterfly_q_297_MoS2_GGA_G.dat")
    for i in range(len(dataframeVectors["E2q0"][0])):
        print(dataframeVectors["E2q0"][0][i])

    numLevels = 80
    lv1 = "E2q17"
    lv2 = "E2q20"

    listAllEigen = {}
    B_arr = dataframeValues["B_values"].values
    for i in range(numLevels + 1):
        listAllEigen[f"E2q{i}"] = dataframeValues[f"E2q{i}"].values

    return None


if __name__ == "__main__":
    main()
